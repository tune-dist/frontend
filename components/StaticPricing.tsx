        ...
        >
          <div className="w-full overflow-x-auto scrollbar-hide">
            <div className="min-w-[1100px]">
              {/* Table Header */}
              <div className="grid grid-cols-[280px,repeat(5,1fr)] gap-0 items-stretch border-b border-white/10">
                <div className="p-8 flex items-end">
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-[0.2em]">Comparison Matrix</p>
                </div>
                {plans.map((plan) => (
                  <div
                    key={plan.key}
                    className={`p-8 flex flex-col items-center text-center transition-colors relative ${plan.isPopular ? 'bg-white/[0.03]' : ''}`}
                  >
                    {plan.isPopular && (
                    ...
                    <div className="h-12 flex flex-col justify-center mb-4">
                      {plan.isPopular && (
                        <span className="animated-gradient-bg text-white text-[10px] uppercase font-black tracking-widest px-3 py-1 rounded-full shadow-lg mb-2">
                          Most Popular
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-baseline gap-1 mb-6">
                      <span className="text-2xl font-bold text-white">{plan.price.split('/')[0]}</span>
                      {plan.price.includes('/') && (
              ...
              <div className="pb-12">
                {categories.map((category) => (
                  <div key={category.name}>
                    <div className="bg-white/[0.05] border-y border-white/5 px-8 py-4">
                      <h4 className="text-xs font-black text-white/40 tracking-[0.3em] uppercase">{category.name}</h4>
                    </div>
                    {category.features.map((feature, fIdx) => (
                      <div
                        key={feature.name}
                        className="grid grid-cols-[280px,repeat(5,1fr)] gap-0 group transition-colors hover:bg-white/[0.01]"
                      >
                        ...
                        {feature.values.map((val, idx) => (
                          <div
                            key={idx}
                            className={`px-4 py-4 flex justify-center border-r border-white/5 last:border-r-0 ${plans[idx].isPopular ? 'bg-purple-700/[0.3]' : ''}`}
                          >
                            {typeof val === 'boolean' ? (
                              <div className={`flex items-center justify-center h-8 w-8 rounded-full ${val ? 'bg-emerald-500/10' : 'bg-white/5'}`}>
                                {val ? (
                                  <Check className="h-4 w-4 text-emerald-500" strokeWidth={3} />
                                ) : (
                                  <X className="h-3 w-3 text-white/10" strokeWidth={3} />
                                )}
                              </div>
                            ) : (
                              <span className={`text-sm font-semibold tracking-tight
                                ${feature.types && feature.types[idx] === 'badge-green' ? 'bg-emerald-500/10 text-emerald-500 px-3 py-1 rounded-full text-[11px] border border-emerald-500/20' : ''}
                                ${feature.types && feature.types[idx] === 'badge-orange' ? 'bg-orange-500/10 text-orange-500 px-3 py-1 rounded-full text-[11px] border border-orange-500/20' : ''}
                                ${val === 'Add-on' ? 'text-primary underline underline-offset-4 decoration-primary/30' : 'text-white/80'}
                                ${val === 'Unlimited' ? 'text-white bg-white/10 px-3 py-1 rounded-full text-[11px]' : ''}
                              `}>
                                {val}
                              </span>
                            )}
